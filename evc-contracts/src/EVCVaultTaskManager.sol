...
contract EVCVaultTaskManager is ERC4626, IVault, IBorrowableEVCVault, BLSSignatureChecker, Initializable {
    ...
    ) ERC4626(_asset) ERC20(_name, _symbol) BLSSignatureChecker(_registryCoordinator) {
        evc = _evc;
    ...
    function initialize(address _aggregator, address _generator) public initializer {
    ...
    // The account status check might not be necessary in certain situations.
    ...
    // This function doesnot retrieve the message sender in the context of the EVC for a borrow operation
    ...
    // The function reverts if the vault is not enabled as a controller
    ...
    function checkAccountStatus(
        ...
        for (uint256 i = 0; i < collaterals.length; ++i) {
            if (collaterals[i] == address(this)) {
                ...
                uint256 maxLiability = (collateral * 9) / 10; // Maximum liability at 90%
    ...
    // Snapshot functionality, for vault status check to have the initial state of the vault in order to evaluate the
    // vault health, and access the snapshot to compare it with the current vault state
