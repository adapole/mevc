...
import
    "../lib/eigenlayer-middleware/lib/eigenlayer-contracts/lib/openzeppelin-contracts/contracts/proxy/transparent/ProxyAdmin.sol";
...
import {ERC20Mock, IERC20} from "../src/ERC20Mock.sol";
...
// forge script script/CredibleSquaringDeployer.s.sol:CredibleSquaringDeployer --rpc-url $RPC_URL  --private-key
// $PRIVATE_KEY --broadcast -vvvv
contract IncredibleSquaringDeployer is Script, Utils {
    ...
    // ERC20 and Strategy: we need to deploy this erc20, create a strategy for it, and whitelist this strategy in the
    // strategymanager
    ...
    function _deployErc20AndStrategyAndWhitelistStrategy(
        ...
        // TODO(samlaf): any reason why we are using the strategybase with tvl limits instead of just using
        // strategybase?
    ...
    function _deployCredibleSquaringContracts(
        ...
        /**
         * First, deploy upgradeable proxy contracts that **will point** to the implementations. Since the
         * implementation contracts are
         * not yet deployed, we give these proxies an empty contract as the initial implementation, to act as if they
         * have no code.
         */
            ...
            for (uint256 i = 0; i < numQuorums; i++) {
                ...
                for (uint256 j = 0; j < numStrategies; j++) {
                    quorumsStrategyParams[i][j] = IStakeRegistry.StrategyParams({
                        ...
                        // see
                        // https://github.com/Layr-Labs/eigenlayer-middleware/blob/m2-mainnet/src/StakeRegistry.sol#L484
